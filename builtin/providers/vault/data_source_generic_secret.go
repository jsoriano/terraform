...
func runVaultCommand(c *api.Client, command, path, wrapTTL string) (*api.Secret, error) {
	if wrapTTL != "" {
	...
	case "write":
		return c.Logical().Write(path, nil)
	}
...
func genericSecretDataSourceRead(d *schema.ResourceData, meta interface{}) error {
	...
	secret, err := runVaultCommand(client, command, path, wrapTTL)
	if err != nil {
